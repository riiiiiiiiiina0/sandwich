...
export const createIframeMenu = (iframeWrapper, index, totalCount) => {
  ...
  fullPageBtn.title = `Full screen (${modifierKey}+F)`;
  ...
  fullPageBtn.addEventListener('click', () => {
    if (isFullPage(iframeWrapper)) {
      ...
      fullPageBtn.title = `Quit full screen (${modifierKey}+F)`;
    }
  ...
  if (index > 0) {
    ...
    moveLeftBtn.title =
      (isVerticalLayout ? 'Move up' : 'Move left') + ` (${modifierKey}+A)`;
  ...
  if (index < totalCount - 1) {
    ...
    moveRightBtn.title =
      (isVerticalLayout ? 'Move down' : 'Move right') + ` (${modifierKey}+D)`;
  ...
  removeBtn.title = `Remove (${modifierKey}+X)`;
  ...
  detachBtn.title = `Open in new tab (${modifierKey}+E)`;
